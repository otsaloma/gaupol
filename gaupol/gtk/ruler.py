...
class _Ruler(object):
    ...
    def _update_length_unit(self):
        ...
        self._length_unit = gaupol.gtk.conf.editor.length_unit
    ...
    def get_em_length(self, text, strip=False, floor=False):
        ...
        if strip: text = gaupol.re_any_tag.sub("", text)
        text = text.replace("\n", " ")
    ...
    def get_lengths(self, text, strip=False, floor=False):
        ...
        if self._length_unit == gaupol.gtk.length_units.CHAR:
            return self.get_char_lengths(text, strip, floor)
        if self._length_unit == gaupol.gtk.length_units.EM:
...
def get_length_function(unit):
    ...
    if unit == gaupol.gtk.length_units.CHAR:
        return _ruler.get_char_length
    if unit == gaupol.gtk.length_units.EM:
