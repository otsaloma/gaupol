...
class Extension(object):
    ...
    def read_config(self, spec_file):
        ...
        The configurations are read from the global aeidon configuration file
        according to the extension-specific specification file 'spec_file'.
        Options are stored as global variables under gaupol.conf and are
        written to the global configuration file automatically.
        """
        config_file = gaupol.conf.config_file
        config = gaupol.Config(config_file, spec_file, False)
        dummy = gaupol.Config(None, spec_file, False)
        ...
        gaupol.conf.extensions.update(config["extensions"])
    ...
    def show_help(self):
        ...
        aeidon.util.browse_url to view HTML documentation.
        """
