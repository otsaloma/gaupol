...
class MicroDVD(TagLibrary):
    ...
    decode_tags = [
        ...
        (r"\{y:(b|i|u)[^\}]*?(b|i|u)[^\}]*?(b|i|u)\}(.*?)$", _FLAGS,
            r"<\1><\2><\3>\4</\3></\2></\1>"),
        # Style x2 (single line)
        (r"\{y:(b|i|u)[^\}]*?(b|i|u)\}(.*?)$", _FLAGS,
            r"<\1><\2>\3</\2></\1>", 2),
        # Style x1 (single line)
        (r"\{y:(b|i|u)\}(.*?)$", _FLAGS,
            r"<\1>\2</\1>", 3),
        # Style x3 (whole subtitle)
        (r"\{Y:(b|i|u)[^\}]*?(b|i|u)[^\}]*?(b|i|u)\}(.*?)\Z", _FLAGS,
            r"<\1><\2><\3>\4</\3></\2></\1>"),
        # Style x2 (whole subtitle)
        (r"\{Y:(b|i|u)[^\}]*?(b|i|u)\}(.*?)\Z", _FLAGS,
            r"<\1><\2>\3</\2></\1>", 2),
        # Style x1 (whole subtitle)
        (r"\{Y:(b|i|u)\}(.*?)\Z", _FLAGS,
            r"<\1>\2</\1>", 3),
        # Color (single line)
        (r"\{c:\$([a-zA-Z0-9]{6})\}(.*?)$", _FLAGS,
            r'<color="#\1">\2</color>'),
        # Color (whole subtitle)
        (r"\{C:\$([a-zA-Z0-9]{6})\}(.*?)\Z", _FLAGS,
            r'<color="#\1">\2</color>'),
        # Font (single line)
        (r"\{f:(.*?)\}(.*?)$", _FLAGS,
            r'<font="\1">\2</font>'),
